...
class gt_twitter_widget extends WP_Widget {
	...
	function gt_twitter_widget() {
		...
		$widget_ops = array(
			...
			'description' => __('Display a list of your latest Tweets', 'golden')
		);

		$control_ops = array();

		$this->WP_Widget('gt_twitter_widget', __('Twitter', 'golden'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		$gtoutput = '
			<p style="border-bottom: 1px solid #DFDFDF;">
				<label for="' . $this->get_field_id('gt_twitter_title') . '"><strong>' . __('Title', 'golden') . '</strong></label>
			</p>
			<p>
		...
		// Settings
		$gtoutput = '
			<p style="border-bottom: 1px solid #DFDFDF;"><strong><label>' . __('Username', 'golden') . '</label></strong></p>
			...
			<p style="border-bottom: 1px solid #DFDFDF;"><strong><label>' . __('Number of tweets to show', 'golden') . '</label></strong></p>
			...
			<p style="border-bottom: 1px solid #DFDFDF;"><strong><label>' . __('Duration of cache', 'golden') . '</label></strong></p>
				<input style="text-align:right;" id="' . $this->get_field_id('gt_twitter_cache_duration') . '" name="' . $this->get_field_name('gt_twitter_cache_duration') . '" type="text" size="10" value="' . $instance['gt_twitter_cache_duration'] . '" /> '.__('Seconds', 'golden').'
			</p>
			
			<p>
				<label>' . __('Show your avatar?', 'golden') . ' 
	...
	function gt_twitter_output($args = array(), $position) {
		...
		add_filter( 'wp_feed_cache_transient_lifetime' , 'gt_tw_filter_handler' ); 
			...
			function gttw_format_since($date){
				...
				if($diff < 60 ) {
					$the_date .= $diff.' ';
					$the_date .= ($diff > 1) ?  __('Seconds ago', 'golden') :  __('Second ago', 'golden');
				}
				elseif($diff < 3600 ) {
					$the_date .= round($diff/60).' ';
					$the_date .= (round($diff/60) > 1) ?  __('Minutes ago', 'golden') :  __('Minute ago', 'golden');
				}
				elseif($diff < 86400 ) {
					$the_date .=  round($diff/3600).' ';
					$the_date .= (round($diff/3600) > 1) ?  __('Hours ago', 'golden') :  __('Hour ago', 'golden');
				}
				else {
					$the_date .=  round($diff/86400).' ';
					$the_date .= (round($diff/86400) > 1) ?  __('Days ago', 'golden') :  __('Day ago', 'golden');
				}
			...
			function gttw_get_the_user_timeline($username, $nb_tweets, $show_avatar) {
				...
				if ( $the_best_feed ) {
					...
					foreach ( $rss_i as $tweet ) {
						...
						$html_avatar = ($show_avatar && $avatar) ? '<a class="tweet_avatar" href="http://twitter.com/' . $username . '" title="' . __('Follow', 'golden') . ' @'.$username.' ' . __('on twitter.', 'golden') . '"><img src="'.$avatar.'" alt="'.$username.'" width="48" height="48" /></a>' : '';
						$xml_result .= '
							<li>
								'.$html_avatar.'
				...
				// if any feed is readable
					...
					$xml_result = '<li><em>'.__('The RSS feed for this twitter account is not loadable for the moment.', 'golden').'</em></li>';
			...
			echo '
				...
				<p class="gt_twitter_follow_us" style="margin: 10px 0;"> 
						<span class="gt_tw_follow"><i class="icon-twitter"></i> ' . __('Follow', 'golden') . '</span>
						<a class="gt_tw_username" href="http://twitter.com/' . $the_username . '">@' . $the_username . '</a>
						<span class="gt_tw_ontwitter">' . __('on Twitter.', 'golden') . '</span>
					</p>
			';
	}
}
