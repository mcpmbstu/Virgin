...
function team_register() {
    $labels = array(
        ...
        'add_new_item' => __('Add New Team Member', 'golden'),
        'edit_item' => __('Edit Team Member', 'golden'),
        'new_item' => __('New Team Member', 'golden'),
        'view_item' => __('View Team Member', 'golden'),
        'search_items' => __('Search Team Members', 'golden'),
...
function team_help_text($contextual_help, $screen_id, $screen) {
    if ('team' == $screen->id) {
        $contextual_help =
        '<h3>' . __('Things to remember when adding a Team Member:', 'golden') . '</h3>' .
        '<ul>' .
        '<li>' . __('Add your Team Member name. (ie; Lana Del Rey).', 'golden') . '</li>' .
        '<li>' . __('Add a short description (Bio) for your member.', 'golden') . '</li>' .
        '<li>' . __('Add a featured image (headshot possibly) to appear above your team member.', 'golden') . '</li>' .
        '<li>' . __('Add a Job Title for your member. (ie; The Boss or Managing Director).', 'golden') . '</li>' .
        '<li>' . __('Add any Social Networks your Team Member belongs to. (ie; Twitter or Linkedin).', 'golden') . '</li>' .
        '</ul>';
    }
    elseif ('edit-team' == $screen->id) {
        $contextual_help = '<p>' . __('A list of all team members appear below. To edit a member, click on the Team Member name.', 'golden') . '</p>';
    }
...
function team_image_box() {
	...
	add_meta_box('postimagediv', __('Team Member Image', 'golden'), 'post_thumbnail_meta_box', 'team', 'side', 'low');
}
...
?>
    ...
    <h2><?php _e('Sort Team Members', 'golden'); ?> <img src="<?php echo home_url(); ?>/wp-admin/images/loading.gif" id="loading-animation" /></h2>
    <p><?php _e('Click, drag, re-order. Repeat as neccessary. Team Member at the top will appear first on your page.', 'golden'); ?></p>
    <ul id="post-list">
