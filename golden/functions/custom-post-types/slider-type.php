...
function slider_register() {
    $labels = array(
        'name' => __('Slider', 'golden'),
        'add_new' => __('Add New', 'golden'),
        'add_new_item' => __('Add New Slider Item', 'golden'),
        'edit_item' => __('Edit Slider Item', 'golden'),
        'new_item' => __('New Slider Item', 'golden'),
        'view_item' => __('View Slider Item', 'golden'),
        'search_items' => __('Search Slider Items', 'golden'),
...
function slider_help_text($contextual_help, $screen_id, $screen) {
    if ('slider' == $screen->id) {
        $contextual_help =
        '<h3>' . __('Things to remember when adding a Slider item:', 'golden') . '</h3>' .
        '<ul>' .
        '<li>' . __('Give the item a title. This will appear as the main heading on the slide.', 'golden') . '</li>' .
        '<li>' . __('Add a short caption. This will appear below the main heading on the slide.', 'golden') . '</li>' .
        '<li>' . __('Attach a Featured Image to use on your Slider.', 'golden') . '</li>' .
        '<li>' . __('Add a link to the slider item. This can be a link to a Portfolio item, or an external link.', 'golden') . '</li>' .
        '</ul>';
    }
    elseif ('edit-slider' == $screen->id) {
        $contextual_help = '<p>' . __('A list of all Slider items appear below. To edit an item, click on the items title.', 'golden') . '</p>';
    }
...
function slider_image_box() {
	...
	add_meta_box('postimagediv', __('Slide Image', 'golden'), 'post_thumbnail_meta_box', 'slider', 'side', 'low');
}
...
?>
    ...
    <h2><?php _e('Sort Slider Items', 'golden'); ?> <img src="<?php echo home_url(); ?>/wp-admin/images/loading.gif" id="loading-animation" /></h2>
    <p><?php _e('Click, drag, re-order. Repeat as neccessary. Slide item at the top will appear first on your slider.', 'golden'); ?></p>
    <ul id="post-list">
