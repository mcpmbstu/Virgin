...
function quotes_register() {
    $labels = array(
        'name' => __('Quotes', 'golden'),
        'add_new' => __('Add New', 'golden'),
        'add_new_item' => __('Add New Quote', 'golden'),
        'edit_item' => __('Edit Quote Item', 'golden'),
        'new_item' => __('New Quote Item', 'golden'),
        'view_item' => __('View Quote Item', 'golden'),
        'search_items' => __('Search Quote Items', 'golden'),
...
function quotes_help_text($contextual_help, $screen_id, $screen) {
    if ('quotes' == $screen->id) {
        $contextual_help =
        '<h3>' . __('Things to remember when adding a Quote:', 'golden') . '</h3>' .
        '<ul>' .
        '<li>' . __('Give the Quote a reference name. This will not appear on your site, and is just for reference purposes (ie; Quote from Jules Verne).', 'golden') . '</li>' .
        '<li>' . __('Enter a short quote or testimonial.', 'golden') . '</li>' .
        '<li>' . __('Enter the name of the quote author.', 'golden') . '</li>' .
        '</ul>';
    }
    elseif ('edit-quotes' == $screen->id) {
        $contextual_help = '<p>' . __('A list of all quotes appear below. To edit a quote, click on the quote Reference Name.', 'golden') . '</p>';
    }
...
?>
    ...
    <h2><?php _e('Sort Quote Items', 'golden'); ?> <img src="<?php echo home_url(); ?>/wp-admin/images/loading.gif" id="loading-animation" /></h2>
    <p><?php _e('Click, drag, re-order. Repeat as neccessary. Quote item at the top will appear first on your page.', 'golden'); ?></p>
    <ul id="post-list">
