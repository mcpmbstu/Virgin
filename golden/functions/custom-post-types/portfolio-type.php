...
function portfolio_register() {
    $labels = array(
        'name' => __('Portfolio', 'golden'),
        'add_new' => __('Add New', 'golden'),
        'add_new_item' => __('Add New Portfolio Item', 'golden'),
        'edit_item' => __('Edit Portfolio Item', 'golden'),
        'new_item' => __('New Portfolio Item', 'golden'),
        'view_item' => __('View Portfolio Item', 'golden'),
        'search_items' => __('Search Portfolio Items', 'golden'),
...
function portfolio_help_text($contextual_help, $screen_id, $screen) {
    if ('portfolio' == $screen->id) {
        $contextual_help =
        '<h3>' . __('Things to remember when adding a Portfolio item:', 'golden') . '</h3>' .
        '<ul>' .
        '<li>' . __('Give the item a title. The title will be used as the item\'s headline.', 'golden') . '</li>' .
        '<li>' . __('You can choose to insert either single images, image slideshow, or video, below.', 'golden') . '</li>' .
        '<li>' . __('Enter your portfolio item overview into the Visual or HTML area. The text will appear for your project overview.', 'golden') . '</li>' .
        '<li>' . __('Choose (or first create) a Project Type. You will need to use these to enable the filterable Portfolio option.', 'golden') . '</li>' .
        '</ul>';
    }
    elseif ('edit-portfolio' == $screen->id) {
        $contextual_help = '<p>' . __('A list of all Portfolio items appear below. To edit an item, click on the items\'s title.', 'golden') . '</p>';
    }
...
?>
    ...
    <h2><?php _e('Sort Portfolio Items', 'golden'); ?> <img src="<?php echo home_url(); ?>/wp-admin/images/loading.gif" id="loading-animation" /></h2>
    <p><?php _e('Click, drag, re-order. Repeat as neccessary. Portfolio item at the top will appear first on your page.', 'golden'); ?></p>
    <ul id="post-list">
