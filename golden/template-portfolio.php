				...
				<div <?php post_class("$term_list eight columns"); ?> id="post-<?php the_ID(); ?>">
				    ...
				    <div class="project-item">
				        ...
				        <div class="overlay">
				        	<div class="details">
                            	<?php
								//$thumb = get_field('hover_image');;
								$img_url = get_field('hover_image'); //get img URL
								$image = aq_resize( $img_url, 90, 90, false ); //resize & crop img
								if(!empty($img_url)){
								?>
								<img src="<?php echo $image ?>" class="overlay-img"  />
                                <?php }else{ ?>
                                <img src="<?php echo $image ?>" class="blank-overlay-img"  />
                                <?php } ?>
								<?php echo do_shortcode(get_post_meta($post->ID, 'gt_portfolio_icon', $single = true)) ?>
								<h2><a href="<?php the_permalink() ?>"><?php the_title(); ?></a></h2>
                                <h3 class="overview-button"><a href="<?php the_permalink() ?>">Overview</a></h3>
