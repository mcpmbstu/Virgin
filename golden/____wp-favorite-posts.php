...
function wpfp_list_most_favorited($limit=5) {
    ...
    if ($results) {
        echo "<div style='margin: 0 0 0 20%;'><ul class='list-favs'>";
			echo "<li>";
			echo "<span class='com_fav fav_title header_fav'>Name</span>";
			//echo "<span class='com_fav fav_cat header_fav'>Section</span>";
			echo "<span class='com_fav fav_img header_fav' style='text-align: center;'>Image</span>";
			echo '<span class="com_fav fav_act header_fav" style="text-align: center;">Action</span>';
            echo "</li>";
			
        foreach ($results as $o):
            $p = get_post($o->post_id);
            echo "<li id=post-".$o->post_id." class='list-fav'>";
			echo "<span class='com_fav fav_title' style='padding: 4% 2%;'>".$p->post_title."</span>";
			//echo "<span class='com_fav fav_cat' style='padding: 4% 2%;'>";
				//$tem = get_post( $o->post_id );
				//$id = $o->post_id;
    			//var_dump($tem);
			//echo "</span>";
			echo "<span class='com_fav fav_img'>";
				$feat_image = wp_get_attachment_url( get_post_thumbnail_id($o->post_id) );
				$feat_image = aq_resize( $feat_image, 45, 45, true ); //resize & crop img
				//echo $feat_image;
				echo '<img src='.$feat_image.' title="'.$p->post_title.'" alt="'.$p->post_title.'" />';
			echo "</span>";
			echo '<span class="com_fav fav_act" style="padding: 3.25% 2%;"><a rel="nofollow" title="Remove from favorites" href="?wpfpaction=remove&amp;postid='.$o->post_id.'" class="wpfp-link">x</a></span>';
            //echo "<a href='".get_permalink($o->post_id)."' title='". $p->post_title ."'>" . $p->post_title . "</a> ($o->meta_value)";
			//echo "<a href='".get_permalink($o->post_id)."' title='". $p->post_title ."'>" . $p->post_title . "</a>";
